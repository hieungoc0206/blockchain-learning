...
class Block {
    constructor(data, previousHash, difficulty, timestamp = Date.now()) {
    ...
    mine(difficulty, timestamp) {
        while (parseInt(this.hash, 16) > Math.pow(16, 64 - this.difficulty)) {
            ...
            this.difficulty = this.adjustDifficulty(difficulty, timestamp, this.timestamp)
            this.nonce++
            this.hash = this.calculateHash()
        }
    }

    adjustDifficulty(difficulty, timestamp, newTimestamp) {
